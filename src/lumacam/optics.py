...
class Lens:
    ...
    def _set_iris_to_fnumber(self, opm: "OpticalModel", stop_idx: int) -> None:
        ...
        # Surfaces 0 (object) and last (image) are virtual planes whose 1mm .zmx
        # default is a design placeholder for a point source at infinity — not physical
        # glass. Opening them ensures only the iris and real glass elements clip rays.
        sm = opm.seq_model
        sm.stop_surface = stop_idx
        opm.update_model()
        ax_ray = opm['analysis_results']['parax_data'][0]
        sm.ifcs[stop_idx].set_max_aperture(abs(ax_ray[stop_idx][0]))
        for idx in (0, len(sm.ifcs) - 1):
            try:
                sm.ifcs[idx].set_max_aperture(1000.0)
            except Exception:
                pass
